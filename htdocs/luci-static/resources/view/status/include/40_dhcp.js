...
/*
 * Online Hosts with per-client traffic survey.
 *
 * Data sources:
 *   - host list : luci-rpc getDHCPLeases + /proc/net/arp + host hints
 *   - rates     : ubus luci.client-rates get  (conntrack accounting; with
...
return baseclass.extend({
	...
	clientAddresses(lease, hints) {
		...
		return Array.from(new Set([
			lease.ipaddr,
			...L.toArray(lease.ipaddrs),
			lease.ip6addr,
			...L.toArray(lease.ip6addrs),
			...L.toArray(host.ipaddrs || host.ipv4),
			...L.toArray(host.ip6addrs || host.ipv6)
	...
	initLeaseTable(table) {
		...
		const totalIndex = Array.from(table.querySelectorAll('th'))
			.findIndex(th => th.dataset.totalTraffic);
	...
	updateLeaseOrder(table) {
		...
		if (sorting) {
			const rows = Array.from(table.querySelectorAll('tr'))
				.filter(row => row.querySelector('.luci-client-rate'));
			const key = row => {
				const cell = row.children[sorting[0]];
				return cell.hasAttribute('data-value')
					? Number(cell.dataset.value)
					: widget.deriveSortKey(cell.querySelector('a[data-value]') || cell, sorting[0]);
			};
			const sorted = rows.slice().sort((a, b) => {
				const av = key(a), bv = key(b);
				const cmp = (typeof av == 'number' && typeof bv == 'number')
					? av - bv : L.naturalCompare(av, bv);
	...
	refreshRates() {
		...
		const addresses = Array.from(new Set(
			cells.flatMap(cell => JSON.parse(cell.dataset.addresses))));
		if (!cells.length)
		...
		return L.resolveDefault(callClientRates(addresses.slice(0, 1024)), {}).then(data => {
			...
			document.querySelectorAll('.luci-client-rate').forEach(cell => {
				const value = this.rateValue(
					JSON.parse(cell.dataset.addresses), data,
					cell.dataset.direction, cell.dataset.mac);
				cell.textContent = value[1];
				cell.closest('td')?.setAttribute('data-value', value[0]);
			});
			document.querySelectorAll('#status_leases')
				.forEach(table => this.updateLeaseOrder(table));
		});
	},
	...
	render([ dhcp_leases, host_hints, dhcp_config, arp ]) {
		if (!this.ratePoll) {
	...
	arpLeases(arp) {
		...
		for (const line of String(arp || '').trim().split(/\n/)) {
			...
			if (fields.length != 6 || fields[5] != 'br-lan' || !(parseInt(fields[2], 16) & 2) ||
			    !validation.parseIPv4(fields[0]) ||
			    !/^(?:[0-9a-f]{2}:){5}[0-9a-f]{2}$/i.test(fields[3]) ||
			    fields[3] == '00:00:00:00:00:00' ||
			    (parseInt(fields[3].slice(0, 2), 16) & 1))
				continue;
			leases.push({
				ipaddr: fields[0],
				macaddr: fields[3].toUpperCase(),
				interface: fields[5]
			});
		}
		return leases;
	...
	mergeLeases(leases, leases6, hints) {
		...
		const addresses = lease => [
			lease.ipaddr, ...L.toArray(lease.ipaddrs),
			lease.ip6addr, ...L.toArray(lease.ip6addrs)
		].filter(Boolean);
		...
		for (const [ mac, host ] of Object.entries(hints.hosts || {}))
			for (const ip of [ ...L.toArray(host.ipaddrs || host.ipv4),
					   ...L.toArray(host.ip6addrs || host.ipv6) ])
		...
		for (const [ family, list ] of [ [ 4, leases ], [ 6, leases6 ] ]) {
			for (const lease of list) {
				...
				const unknown = lease.duid
					? [ family, lease.duid.toLowerCase(), lease.iaid?.toLowerCase(), lease.interface ]
					: addresses(lease).length
						? [ family, ...addresses(lease).map(ip => this.normalizeRateAddress(ip)) ]
						: null;
				const key = mac || (unknown ? JSON.stringify(unknown) : Symbol());
				if (!clients.has(key))
					clients.set(key, {
						macaddr: mac, hostname: lease.hostname,
						ipaddrs: [], ip6addrs: [], interfaces: [],
						leases: [], leases6: [], activeAddresses: []
					});
				...
				if (!lease._historical && !records.some(record =>
						family == 4 ? record.ipaddr == lease.ipaddr
						            : record.duid?.toLowerCase() == lease.duid?.toLowerCase() &&
						              record.iaid == lease.iaid))
		...
		for (const client of clients.values()) {
			...
			client.rateAddresses = client.activeAddresses.length
				? this.clientAddresses(current, hints) : [];
		}
		...
		for (const [ family, leases ] of [ [ 4, client.leases ], [ 6, client.leases6 ] ]) {
			for (const lease of leases) {
				...
				if (family == 4 ? !mac : !duid)
					continue;
				const disabled = family == 4
					? this.isMACStatic[mac]
					: this.isDUIDStatic[duid] || (iaid && this.isDUIDIAIDStatic[`${duid}%${iaid}`]);
				actions.push(E('button', {
					'class': 'cbi-button cbi-button-apply',
					'click': L.bind(family == 4 ? this.handleCreateStaticLease
					                            : this.handleCreateStaticLease6, this, lease),
					'data-tooltip': _('Reserve a specific IP address for this device'),
					'disabled': disabled || null
				}, [ _('Reserve IP') + ` (IPv${family})` + (leases.length > 1
					? ` ${family == 4 ? lease.ipaddr : lease.ip6addrs?.[0] || lease.ip6addr || ''}` : '') ]));
			}
		}
		return E('div', { 'style': 'display:flex;flex-direction:column;gap:4px;align-items:center' }, actions);
	},

	renderLeases(dhcp_leases, host_hints, arp) {
		const leases = [
			...(Array.isArray(dhcp_leases.dhcp_leases) ? dhcp_leases.dhcp_leases : []),
			...this.arpLeases(arp)
		];
		...
		cbi_update_table(table, clients.map(client => {
			...
			return [
				E('span', {
					'class': 'badge',
					'style': online
						? 'background:#2ea44f;color:#fff;border-radius:50%;width:10px;height:10px;display:inline-block'
						: 'background:#999;color:#fff;border-radius:50%;width:10px;height:10px;display:inline-block',
					'title': online ? _('Online') : _('Offline')
				}),
				E('span', { 'style': 'display:block;text-align:left' }, host || '-'),
				v4,
				v6,
				client.macaddr || '-',
				...
				isReadonlyView ? E([]) : this.leaseActions(client)
			];
		}));
